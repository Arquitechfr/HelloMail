...
│   ├── auth/       authService, twoFactorService (TOTP), webauthnService (passkeys login complet Phase 11), oauthService (Google XOAUTH2), microsoftOAuthService (Microsoft XOAUTH2, Phase 7)
│   ├── email/      connectionTest, imapPool (XOAUTH2 Google/Microsoft), sanitize, messageFetchService (détail + unsubscribeInfo Phase 16 + PEEK + parsing plié), attachmentService (PJ + stream RFC 822 .eml), threadService (regroupement inReplyTo + sujet), sendService (XOAUTH2 + MDN + aliases From/Sender RFC 5322 Phase 14), receiptService (RFC 3798, Phase 8 + 15.1 idempotent), ruleService (moteur de règles, Phase 8), tagService (gestion libellés & propagation cascade, Phase 9), snoozeService (mise en sommeil & boucle de réveil, Phase 9), pinService (épinglage en tête de liste, Phase 12), unifiedMessagesService (agrégation dossiers unifiés, Phase 12), scheduledEmailService (programmation d'envois, Phase 13), scheduledEmailRunner (runner worker périodique Send Later, Phase 13), unsubscribeService (désabonnement 1-clic RFC 2369/8058, Phase 16), blockSenderService (blocage expéditeur 1-clic & règle spam, Phase 16), importEmailService (importation directe RFC 822 .eml, Phase 16), folderService (+ cache Folder en base + folderExists, Phase 10 + folderProtection Phase 11), specialFolders, messageActionService, searchService, imapSearchService (fallback recherche serveur IMAP, Phase 10), draftService (avec pièces jointes multipart/mixed Phase 13), fetchMoreService
...
│   ├── contacts/   contactService (CRUD + recherche/autocomplétion, Phase 6 ; addSenderContactIfEnabled opt-in Phase 10 + 15.1), contactImportExportService (vCard RFC 6350 & CSV RFC 4180, Phase 11)
│   ├── sync/       syncManager, accountRegistry (tous providers + lock distribué, Phase 10 + runner scheduled emails Phase 13), syncLock (lock Redis SET NX PX + token + renew, Phase 10), initialSync (+ delta CONDSTORE changedSince, Phase 10), idleLoop, pollingSync, reconcileFolder, reconcileAllFolders, messageMapper
│   ├── observability/ metricsService (Prometheus prom-client, Phase 6), workerHeartbeat (heartbeat Redis → /api/health, Phase 10)
│   ├── realtime/   eventPublisher (Redis Pub/Sub worker→API), eventSubscriber (filtrage par userId)
│   └── accounts/   accountService (create, list, delete, toggle, updateSignature Phase 7, color Phase 12), autoconfigService (ISPDB / MX, Phase 7), aliasService (CRUD sous-document alias, Phase 14)
├── controllers/    authController (+ updatePreferences Phase 9+12), twoFactorController, oauthController (Google + Microsoft Phase 7), accountsController (signature, autoconfig Phase 7, aliases Phase 14), messagesController (+ getRaw, getThread, sendReceipt, snooze Phase 9, pin Phase 12), scheduledController (Phase 13), unsubscribeController (Phase 16), blockSenderController (Phase 16), importEmailController (Phase 16), pgpController (Phase 15), tagsController (Phase 9), templatesController (Phase 9), rulesController (Phase 8), foldersController, draftsController, contactsController (+ import/export Phase 11), healthController, eventsController
├── routes/         authRoutes (+ preferences Phase 9+12), twoFactorRoutes, accountsRoutes (+ autoconfig, signature Phase 7, aliases Phase 14, scheduled Phase 13), messagesRoutes (+ raw, thread, receipt, snooze, pin, unsubscribe, block-sender, import), pgpRoutes (Phase 15), tagsRoutes (Phase 9), templatesRoutes (Phase 9), rulesRoutes (Phase 8), foldersRoutes, draftsRoutes, oauthRoutes (Google + Microsoft Phase 7), contactsRoutes (+ import/export Phase 11), eventsRoutes
