...
├── models/         User (2FA TOTP + WebAuthn, Phase 6, preferences Phase 9 + autoAddContacts Phase 10, defaultsSeededAt presets Phase 9), RefreshToken (rotation + TTL), Account (multi-provider, hook pre-validate, OAuth Google & Microsoft XOAUTH2, signatures Phase 7), Message (inReplyTo, index textuel, indexes conversation, tags Phase 9, snoozedUntil Phase 9), Contact (index textuel + unique, Phase 6), Rule (moteur de tri, Phase 8, isPreset Phase 9), Tag (libellés colorés, Phase 9, isPreset), Template (modèles d'emails & réponses types, Phase 9, isPreset), Folder (cache liste dossiers IMAP, Phase 10), FolderSyncState (état CONDSTORE uidValidity+highestModseq, Phase 10), MessageBody (cache corps text/html, TTL 30 j, Phase 10)
...
│   ├── email/      connectionTest, imapPool (XOAUTH2 Google/Microsoft), sanitize, messageFetchService, attachmentService (PJ + stream RFC 822 .eml), threadService (regroupement inReplyTo + sujet), sendService (XOAUTH2 + MDN), receiptService (RFC 3798, Phase 8), ruleService (moteur de règles, Phase 8), tagService (gestion libellés & propagation cascade, Phase 9), snoozeService (mise en sommeil & boucle de réveil, Phase 9), folderService (+ cache Folder en base + folderExists, Phase 10), specialFolders, messageActionService, searchService, imapSearchService (fallback recherche serveur IMAP, Phase 10), draftService, fetchMoreService
...
│   ├── contacts/   contactService (CRUD + recherche/autocomplétion, Phase 6 ; addSenderContactIfEnabled opt-in Phase 10)
│   ├── sync/       syncManager, accountRegistry (tous providers + lock distribué, Phase 10), syncLock (lock Redis SET NX PX + token + renew, Phase 10), initialSync (+ delta CONDSTORE changedSince, Phase 10), idleLoop, pollingSync, reconcileFolder, reconcileAllFolders, messageMapper
